...
def angle2class(angle):
    ...
    residual_angle = shifted_angle - (class_id * angle_per_class + 
        angle_per_class / 2)
...
def draw_msra_gaussian(heatmap, center, sigma):
    ...
    heatmap[img_y[0]:img_y[1], img_x[0]:img_x[1]] = np.maximum(heatmap[
        img_y[0]:img_y[1], img_x[0]:img_x[1]], g[g_y[0]:g_y[1], g_x[0]:g_x[1]])
    return heatmap
...
def draw_projected_box3d(image, corners3d, color=(255, 255, 255), thickness=1):
    ...
    for k in range(0, 4):
        i, j = k, (k + 1) % 4
        cv2.line(image, (corners3d[i, 0], corners3d[i, 1]), (corners3d[j, 0
            ], corners3d[j, 1]), color, thickness, lineType=cv2.LINE_AA)
        i, j = k + 4, (k + 1) % 4 + 4
        cv2.line(image, (corners3d[i, 0], corners3d[i, 1]), (corners3d[j, 0
            ], corners3d[j, 1]), color, thickness, lineType=cv2.LINE_AA)
        i, j = k, k + 4
