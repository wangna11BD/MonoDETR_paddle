...
def clean_data_by_distance(gt_anno, dt_anno, current_class, difficulty):
    ...
    for i in range(num_gt):
        ...
        if DISTANCE_COVER:
            if gt_anno['occluded'][i] > MAX_OCCLUSION[2] or gt_anno['truncated'
                ][i] > MAX_TRUNCATION[2] or height <= MIN_HEIGHT[2
            ...
            if gt_anno['occluded'][i] > MAX_OCCLUSION[2] or gt_anno['truncated'
                ...
                ignore = True
        elif gt_anno['occluded'][i] > MAX_OCCLUSION[2] or gt_anno['truncated'][
            i] > MAX_TRUNCATION[2] or height <= MIN_HEIGHT[2
            ] or dis > MAX_DISTANCE[difficulty] or dis <= MAX_DISTANCE[
            difficulty - 1]:
            ignore = True
        if valid_class == 1 and not ignore:
            ignored_gt.append(0)
...
def image_box_overlap(boxes, query_boxes, criterion=-1):
    ...
    for k in range(K):
        qbox_area = (query_boxes[k, 2] - query_boxes[k, 0]) * (query_boxes[
            k, 3] - query_boxes[k, 1])
        for n in range(N):
            ...
            if iw > 0:
                ...
                if ih > 0:
                    if criterion == -1:
                        ua = (boxes[n, 2] - boxes[n, 0]) * (boxes[n, 3] -
                            boxes[n, 1]) + qbox_area - iw * ih
                    elif criterion == 0:
                        ua = (boxes[n, 2] - boxes[n, 0]) * (boxes[n, 3] -
                            boxes[n, 1])
                    elif criterion == 1:
...
def d3_box_overlap(boxes, qboxes, criterion=-1):
    rinc = rotate_iou_gpu_eval(boxes[:, ([0, 2, 3, 5, 6])], qboxes[:, ([0, 
        2, 3, 5, 6])], 2)
...
def _prepare_data(gt_annos, dt_annos, current_class, difficulty, DIForDIS=True
    ...
    for i in range(len(gt_annos)):
        rets = clean_data(gt_annos[i], dt_annos[i], current_class, difficulty
            ) if DIForDIS else clean_data_by_distance(gt_annos[i], dt_annos
            [i], current_class, difficulty)
...
def eval_class(gt_annos, dt_annos, current_classes, difficultys, metric,
    min_overlaps, compute_aos=False, num_parts=50, DIForDIS=True):
    ...
    for m, current_class in enumerate(current_classes):
        for l, difficulty in enumerate(difficultys):
            ...
            for k, min_overlap in enumerate(min_overlaps[:, (metric), (m)]):
                ...
                for i in range(len(thresholds)):
                    precision[m, l, k, i] = np.max(precision[(m), (l), (k),
                        i:], axis=-1)
                    recall[m, l, k, i] = np.max(recall[(m), (l), (k), i:],
                        axis=-1)
                    if compute_aos:
                        aos[m, l, k, i] = np.max(aos[(m), (l), (k), i:],
                            axis=-1)
...
def do_eval(gt_annos, dt_annos, current_classes, min_overlaps, compute_aos=
    False, PR_detail_dict=None, DIForDIS=True):
    difficultys = [0, 1, 2]
    ...
    return (mAP_bbox, mAP_bev, mAP_3d, mAP_aos, mAP_bbox_R40, mAP_bev_R40,
        mAP_3d_R40, mAP_aos_R40)
...
def do_coco_style_eval(gt_annos, dt_annos, current_classes, overlap_ranges,
    ...
    for i in range(overlap_ranges.shape[1]):
        for j in range(overlap_ranges.shape[2]):
            min_overlaps[:, (i), (j)] = np.linspace(*overlap_ranges[:, (i),
                (j)])
    mAP_bbox, mAP_bev, mAP_3d, mAP_aos = do_eval(gt_annos, dt_annos,
        current_classes, min_overlaps, compute_aos)
    mAP_bbox = mAP_bbox.mean(axis=-1)
    mAP_bev = mAP_bev.mean(axis=-1)
    mAP_3d = mAP_3d.mean(axis=-1)
    if mAP_aos is not None:
        mAP_aos = mAP_aos.mean(axis=-1)
    return mAP_bbox, mAP_bev, mAP_3d, mAP_aos


def get_official_eval_result(gt_annos, dt_annos, current_classes,
    PR_detail_dict=None):
    overlap_0_7 = np.array([[0.7, 0.5, 0.5, 0.7, 0.5, 0.7], [0.7, 0.5, 0.5,
        0.7, 0.5, 0.7], [0.7, 0.5, 0.5, 0.7, 0.5, 0.7]])
    overlap_0_5 = np.array([[0.7, 0.5, 0.5, 0.7, 0.5, 0.5], [0.5, 0.25, 
        0.25, 0.5, 0.25, 0.5], [0.5, 0.25, 0.25, 0.5, 0.25, 0.5]])
    min_overlaps = np.stack([overlap_0_7, overlap_0_5], axis=0)
    class_to_name = {(0): 'Car', (1): 'Pedestrian', (2): 'Cyclist', (3):
        'Van', (4): 'Person_sitting', (5): 'Truck'}
    ...
    (mAPbbox, mAPbev, mAP3d, mAPaos, mAPbbox_R40, mAPbev_R40, mAP3d_R40,
        mAPaos_R40) = (do_eval(gt_annos, dt_annos, current_classes,
        min_overlaps, compute_aos, PR_detail_dict=PR_detail_dict, DIForDIS=
        True))
    ret_dict = {}
    for j, curcls in enumerate(current_classes):
        for i in range(min_overlaps.shape[0]):
            ...
            result += print_str(
                f'bbox AP:{mAPbbox[j, 0, i]:.4f}, {mAPbbox[j, 1, i]:.4f}, {mAPbbox[j, 2, i]:.4f}'
                )
            result += print_str(
                f'bev  AP:{mAPbev[j, 0, i]:.4f}, {mAPbev[j, 1, i]:.4f}, {mAPbev[j, 2, i]:.4f}'
                )
            result += print_str(
                f'3d   AP:{mAP3d[j, 0, i]:.4f}, {mAP3d[j, 1, i]:.4f}, {mAP3d[j, 2, i]:.4f}'
                )
            if compute_aos:
                result += print_str(
                ...
                if i == 0:
                    ret_dict['%s_aos_easy' % class_to_name[curcls]] = mAPaos[
                        j, 0, 0]
                    ret_dict['%s_aos_moderate' % class_to_name[curcls]
                        ] = mAPaos[j, 1, 0]
                    ret_dict['%s_aos_hard' % class_to_name[curcls]] = mAPaos[
                        j, 2, 0]
            result += print_str(
                f'{class_to_name[curcls]} AP_R40@{{:.2f}}, {{:.2f}}, {{:.2f}}:'
                .format(*min_overlaps[(i), :, (j)]))
            result += print_str(
                f'bbox AP:{mAPbbox_R40[j, 0, i]:.4f}, {mAPbbox_R40[j, 1, i]:.4f}, {mAPbbox_R40[j, 2, i]:.4f}'
                )
            result += print_str(
                f'bev  AP:{mAPbev_R40[j, 0, i]:.4f}, {mAPbev_R40[j, 1, i]:.4f}, {mAPbev_R40[j, 2, i]:.4f}'
                )
            result += print_str(
                f'3d   AP:{mAP3d_R40[j, 0, i]:.4f}, {mAP3d_R40[j, 1, i]:.4f}, {mAP3d_R40[j, 2, i]:.4f}'
                )
            if compute_aos:
                result += print_str(
                    f'aos  AP:{mAPaos_R40[j, 0, i]:.2f}, {mAPaos_R40[j, 1, i]:.2f}, {mAPaos_R40[j, 2, i]:.2f}'
                    )
                if i == 0:
                    ret_dict['%s_aos_easy_R40' % class_to_name[curcls]
                        ] = mAPaos_R40[j, 0, 0]
                    ret_dict['%s_aos_moderate_R40' % class_to_name[curcls]
                        ] = mAPaos_R40[j, 1, 0]
                    ret_dict['%s_aos_hard_R40' % class_to_name[curcls]
                        ] = mAPaos_R40[j, 2, 0]
            if i == 0:
                ret_dict['%s_3d_easy' % class_to_name[curcls]] = mAP3d[j, 0, 0]
                ret_dict['%s_3d_moderate' % class_to_name[curcls]] = mAP3d[
                    j, 1, 0]
                ret_dict['%s_3d_hard' % class_to_name[curcls]] = mAP3d[j, 2, 0]
                ret_dict['%s_bev_easy' % class_to_name[curcls]] = mAPbev[j,
                    0, 0]
                ret_dict['%s_bev_moderate' % class_to_name[curcls]] = mAPbev[
                    j, 1, 0]
                ret_dict['%s_bev_hard' % class_to_name[curcls]] = mAPbev[j,
                    2, 0]
                ret_dict['%s_image_easy' % class_to_name[curcls]] = mAPbbox[
                    j, 0, 0]
                ret_dict['%s_image_moderate' % class_to_name[curcls]
                    ] = mAPbbox[j, 1, 0]
                ret_dict['%s_image_hard' % class_to_name[curcls]] = mAPbbox[
                    j, 2, 0]
                ret_dict['%s_3d_easy_R40' % class_to_name[curcls]] = mAP3d_R40[
                    j, 0, 0]
                ret_dict['%s_3d_moderate_R40' % class_to_name[curcls]
                    ] = mAP3d_R40[j, 1, 0]
                ret_dict['%s_3d_hard_R40' % class_to_name[curcls]] = mAP3d_R40[
                    j, 2, 0]
                ret_dict['%s_bev_easy_R40' % class_to_name[curcls]
                    ] = mAPbev_R40[j, 0, 0]
                ret_dict['%s_bev_moderate_R40' % class_to_name[curcls]
                    ] = mAPbev_R40[j, 1, 0]
                ret_dict['%s_bev_hard_R40' % class_to_name[curcls]
                    ] = mAPbev_R40[j, 2, 0]
                ret_dict['%s_image_easy_R40' % class_to_name[curcls]
                    ] = mAPbbox_R40[j, 0, 0]
                ret_dict['%s_image_moderate_R40' % class_to_name[curcls]
                    ] = mAPbbox_R40[j, 1, 0]
                ret_dict['%s_image_hard_R40' % class_to_name[curcls]
                    ] = mAPbbox_R40[j, 2, 0]
    return result, ret_dict, mAP3d_R40[0, 1, 0]


def get_distance_eval_result(gt_annos, dt_annos, current_classes,
    PR_detail_dict=None):
    overlap_0_7 = np.array([[0.7, 0.5, 0.5, 0.7, 0.5, 0.7], [0.7, 0.5, 0.5,
        0.7, 0.5, 0.7], [0.7, 0.5, 0.5, 0.7, 0.5, 0.7]])
    overlap_0_5 = np.array([[0.7, 0.5, 0.5, 0.7, 0.5, 0.5], [0.5, 0.25, 
    ...
    for j, curcls in enumerate(current_classes):
        for i in range(min_overlaps.shape[0]):
            ...
            if compute_aos:
                ...
                if i == 0:
                    ret_dict['%s_aos_30m' % class_to_name[curcls]] = mAPaos[
                        j, 0, 0]
                    ret_dict['%s_aos_50m' % class_to_name[curcls]] = mAPaos[
                        j, 1, 0]
                    ret_dict['%s_aos_70m' % class_to_name[curcls]] = mAPaos[
                        j, 2, 0]
            result += print_str(
                f'{class_to_name[curcls]} AP_R40@{{:.2f}}, {{:.2f}}, {{:.2f}}:'
                .format(*min_overlaps[(i), :, (j)]))
            ...
            if compute_aos:
                ...
                if i == 0:
                    ret_dict['%s_aos_30m_R40' % class_to_name[curcls]
                        ] = mAPaos_R40[j, 0, 0]
                    ret_dict['%s_aos_50m_R40' % class_to_name[curcls]
                        ] = mAPaos_R40[j, 1, 0]
                    ret_dict['%s_aos_70m_R40' % class_to_name[curcls]
                        ] = mAPaos_R40[j, 2, 0]
            if i == 0:
                ...
                ret_dict['%s_image_30m' % class_to_name[curcls]] = mAPbbox[
                    j, 0, 0]
                ret_dict['%s_image_50m' % class_to_name[curcls]] = mAPbbox[
                    j, 1, 0]
                ret_dict['%s_image_70m' % class_to_name[curcls]] = mAPbbox[
                    j, 2, 0]
                ret_dict['%s_3d_30m_R40' % class_to_name[curcls]] = mAP3d_R40[
                    j, 0, 0]
                ret_dict['%s_3d_50m_R40' % class_to_name[curcls]] = mAP3d_R40[
                    j, 1, 0]
                ret_dict['%s_3d_70m_R40' % class_to_name[curcls]] = mAP3d_R40[
                    j, 2, 0]
                ret_dict['%s_bev_30m_R40' % class_to_name[curcls]
                    ] = mAPbev_R40[j, 0, 0]
                ret_dict['%s_bev_50m_R40' % class_to_name[curcls]
                    ] = mAPbev_R40[j, 1, 0]
                ret_dict['%s_bev_70m_R40' % class_to_name[curcls]
                    ] = mAPbev_R40[j, 2, 0]
                ret_dict['%s_image_30m_R40' % class_to_name[curcls]
                    ] = mAPbbox_R40[j, 0, 0]
                ret_dict['%s_image_50m_R40' % class_to_name[curcls]
                    ] = mAPbbox_R40[j, 1, 0]
                ret_dict['%s_image_70m_R40' % class_to_name[curcls]
                    ] = mAPbbox_R40[j, 2, 0]
...
def get_coco_eval_result(gt_annos, dt_annos, current_classes):
    ...
    class_to_range = {(0): [0.5, 0.95, 10], (1): [0.25, 0.7, 10], (2): [
        0.25, 0.7, 10], (3): [0.5, 0.95, 10], (4): [0.25, 0.7, 10]}
    name_to_class = {v: n for n, v in class_to_name.items()}
    if not isinstance(current_classes, (list, tuple)):
        current_classes = [current_classes]
    current_classes_int = []
