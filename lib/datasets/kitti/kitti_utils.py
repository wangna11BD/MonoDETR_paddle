...
class Object3d(object):
    ...
    def __init__(self, line):
        ...
        self.pos = np.array((float(label[11]), float(label[12]), float(
            label[13])), dtype=np.float32)
    ...
    def to_bev_box2d(self, oblique=True, voxel_size=0.1):
        ...
        if oblique:
            ...
            box2d[:, (1)] = Object3d.BEV_SHAPE[0] - 1 - ((xz_corners[:, (1)
                ] - Object3d.MIN_XZ[1]) / voxel_size).astype(np.int32)
            ...
            cu = np.floor((self.pos[0] - Object3d.MIN_XZ[0]) / voxel_size
                ).astype(np.int32)
            cv = Object3d.BEV_SHAPE[0] - 1 - ((self.pos[2] - Object3d.
                MIN_XZ[1]) / voxel_size).astype(np.int32)
    ...
    def to_str(self):
        print_str = (
            '%s %.3f %.3f %.3f box2d: %s hwl: [%.3f %.3f %.3f] pos: %s ry: %.3f'
             % (self.cls_type, self.trucation, self.occlusion, self.alpha,
            self.box2d, self.h, self.w, self.l, self.pos, self.ry))
        return print_str

    def to_kitti_format(self):
        kitti_str = (
            '%s %.2f %d %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f'
             % (self.cls_type, self.trucation, int(self.occlusion), self.
            alpha, self.box2d[0], self.box2d[1], self.box2d[2], self.box2d[
            3], self.h, self.w, self.l, self.pos[0], self.pos[1], self.pos[
            2], self.ry))
        return kitti_str
...
def get_calib_from_file(calib_file):
    ...
    return {'P2': P2.reshape([3, 4]), 'P3': P3.reshape([3, 4]), 
            'R0': R0.reshape([3, 3]), 'Tr_velo2cam': Tr_velo_to_cam.reshape([3, 4])}
...
class Calibration(object):
    ...
    def rect_to_lidar(self, pts_rect):
        pts_ref = np.transpose(np.dot(np.linalg.inv(self.R0), np.transpose(
            pts_rect)))
    ...
    def corners3d_to_img_boxes(self, corners3d):
        ...
        corners3d_hom = np.concatenate((corners3d, np.ones((sample_num, 8, 
            1))), axis=2)
        img_pts = np.matmul(corners3d_hom, self.P2.T)
        x, y = img_pts[:, :, (0)] / img_pts[:, :, (2)], img_pts[:, :, (1)
            ] / img_pts[:, :, (2)]
        x1, y1 = np.min(x, axis=1), np.min(y, axis=1)
        x2, y2 = np.max(x, axis=1), np.max(y, axis=1)
        boxes = np.concatenate((x1.reshape([-1, 1]), y1.reshape([-1, 1]), x2.
            reshape([-1, 1]), y2.reshape([-1, 1])), axis=1)
        boxes_corner = np.concatenate((x.reshape([-1, 8, 1]), y.reshape([-1, 8,
            1])), axis=2)
    ...
    def flip(self, img_size):
        wsize = 4
        hsize = 2
        p2ds = np.concatenate([np.expand_dims(np.tile(np.expand_dims(np.
            linspace(0, img_size[0], wsize), 0), [hsize, 1]), -1), np.
            expand_dims(np.tile(np.expand_dims(np.linspace(0, img_size[1],
            hsize), 1), [1, wsize]), -1), np.linspace(2, 78, wsize * hsize)
            .reshape(hsize, wsize, 1)], -1).reshape(-1, 3)
...
def get_affine_transform(center, scale, rot, output_size, shift=np.array([0,
    0], dtype=np.float32), inv=0):
...
if __name__ == '__main__':
    ...
    for object in objects:
        ...
        object.pos[0] *= 1
        center_3d = object.pos + [0, -object.h / 2, 0]
        ...
        print('3D center/2D center/projected 3D center:', center_3d,
            center_2d, center_3d_projected)
        print('alpha ---> ry ', object.alpha, calib.alpha2ry(object.alpha,
            center_2d[0]))
        break
